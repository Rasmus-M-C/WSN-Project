...
static void testCconnection()
{
  ...
  //}
  snprintf(str, sizeof(str), "HealthCheck");
  simple_udp_sendto(&udp_conn, str, strlen(str), &dest_ipaddr_C);
  LOG_INFO("Sent HealthCheck to:\n");
  LOG_INFO_6ADDR(&dest_ipaddr_C);
}
...
udp_rx_callback(struct simple_udp_connection *c,
         ...
         uint16_t datalen)
{
  LOG_INFO("Received message from: '%.*s'\n", datalen, (char *) data);
  LOG_INFO_6ADDR(sender_addr);
  LOG_INFO_("\n");
if (strcmp((char *)data, "ACK from C") == 0) {
    direct_connection_to_C = true;
    LOG_INFO("Direct connection to C restablished\n");
  }
